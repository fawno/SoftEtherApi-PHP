...
namespace SoftEtherApi\Infrastructure
{
    ...
    class SHA0
    {
        ...
        public static function ArrayCopyInto(&$source, &$dest, $length, $sourceIndex = 0, $destIndex = 0)
        {
            for ($i = 0; $i < $length; ++$i) {
        ...
        public static function ArrayClear(&$source, $startIndex, $length)
        {
            for ($i = 0; $i < $length; ++$i) {
        ...
        private static function Transform(&$context, &$data, $dataStartIndex, $blocks)
        {
            for ($i = 0; $i < $blocks; ++$i) {
                $wv = array_fill(0, 5, 0);
                $w = array_fill(0, 16, 0);
        ...
        private static function SHA0_R1($x, $y, $z)
        {
            return self::To32BitInteger(($z ^ ($x & ($y ^ $z))) + 0x5a827999);
        }

        private static function SHA0_R2($x, $y, $z)
        {
            return self::To32BitInteger(($x ^ $y ^ $z) + 0x6ed9eba1);
        }

        private static function SHA0_R3($x, $y, $z)
        {
            return self::To32BitInteger((($x & $y) | ($z & ($x | $y))) + 0x8f1bbcdc);
        }

        private static function SHA0_R4($x, $y, $z)
        {
            return self::To32BitInteger(($x ^ $y ^ $z) + 0xca62c1d6);
        }

        private static function SHA0_PRC(&$wv, $a, $b, $c, $d, $e, $idx, $rnd)
        {
            $val = 0;
            switch ($rnd) {
                case 1:
        ...
        private static function SHA0_EXT(&$w, $i)
        {
        ...
        {
            $x = self::To32BitInteger(($buf[$startIndex] << 24)
                ^ ($buf[$startIndex + 1] << 16)
                ^ ($buf[$startIndex + 2] << 8)
                ^ $buf[$startIndex + 3]);
        }
        ...
        private static function To32BitInteger($val)
        {
